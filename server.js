...
function handleAPI(req, res, pathname, params) {
  ...
  if (pathname === "/api/spells") {
    ...
    if (spellIds.length) {
      ...
      const classRows = querySpells("SELECT spell_id, class_name AS name, level FROM spell_classes WHERE spell_id IN (" + ph + ")", spellIds);
      const classMap = {};
      for (const r of classRows) {
        if (!classMap[r.spell_id]) classMap[r.spell_id] = [];
        classMap[r.spell_id].push({ name: r.name, level: r.level });
      }
